...
public enum ControlAudit {
    ...
    public static func summary(of command: ControlCommand) -> String { // swiftlint:disable:this cyclomatic_complexity
        ...
        case let .click(target, count): "click \(describe(target))\(count > 1 ? " ×\(count)" : "")"
        case let .type(text, ref): "type \(text.count) characters\(ref.map { " into \($0)" } ?? "")"
        case let .key(keys): "key \(clip(keys))"
