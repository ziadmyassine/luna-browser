...
struct FrecencyRankingTests {
    ...
    @Test func ordersPlacesBySummedVisitScore() async throws {
        ...
        try await store.recordVisit(url: url("twice-linked"), title: "Twice linked", kind: .link, at: daysAgo(1))
        try await store.recordVisit(url: url("twice-linked"), title: "Twice linked", kind: .link, at: daysAgo(2))
        // 1 × (200 typed × 1.0) = 200
        try await store.recordVisit(url: url("typed-today"), title: "Typed today", kind: .typed, at: daysAgo(1))
        // 1 × (140 bookmarked × 0.5, 20 days ago) = 70
        try await store.recordVisit(url: url("bookmarked"), title: "Bookmarked", kind: .bookmarked, at: daysAgo(20))
        // 1 × (200 typed × 0.1, 200 days ago) = 20
        try await store.recordVisit(url: url("stale"), title: "Stale", kind: .typed, at: daysAgo(200))
        // 3 × (0 redirect) = 0 — a redirect chain must never outrank a real visit.
        for day in 1...3 {
            try await store.recordVisit(url: url("redirects"), title: "Redirects", kind: .redirect, at: daysAgo(Double(day)))
        }

        let hits = try await store.searchHistory("", limit: 10)
    ...
    func weightsOneVisitByItsAge(age: Double, expected: Double) async throws {
        let store = try makeTemporaryStore()
        try await store.recordVisit(url: url("single"), title: "Single", kind: .typed, at: daysAgo(age))

        let hits = try await store.searchHistory("", limit: 1)
