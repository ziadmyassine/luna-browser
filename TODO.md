...
- [ ] **4.6 User-Agent policy** — default to system UA + `applicationNameForUserAgent`. Ship a per-site UA override table (Safari UA / Chrome UA) because some sites gate on Chrome. Add a UI toggle in the site settings pop-out.
...
- [x] **5.15 Private/incognito window** = `WKWebsiteDataStore.nonPersistent()`, visually distinct tint, excluded from history writes, no crash-restore.
...
- [ ] **6.4 Archive browser** (`⌘Y`) with search + restore.
...
- [x] **7.2 Collapse + hover-peek**: `⌘S` collapses to a ~48 px rail (or fully hidden); hovering the left window edge slides the sidebar over the content as a floating panel with a shadow, after a ~0.1 s intent delay, with a ~0.15–0.2 s ease-out reveal. It must **not** trigger while the pointer is merely travelling to the traffic lights.
  ...
  > **Changed 2026-09-24: the peek stays while its pop-out is up.** The Downloads and History lists stand on buttons at the foot of the sidebar and reach past it, so moving the pointer onto a list closed the peek under it. An open Downloads or History list now keeps a peeked sidebar out until the list is closed (`PopoutController.isShowing(in:)`). It never brings a hidden sidebar out by itself: a list a download opens over a hidden sidebar stays in the corner.
  > **Fixed 2026-09-25: the page bar's buttons jumped when the hidden sidebar peeked.** With the search bar on the page and the sidebar hidden, a peek brought the traffic lights back, and the page bar moved its sidebar and back buttons 78 pt right to make room for them — under the sidebar, which is where the lights actually are. The bar now ignores the lights while a leading sidebar is hidden (`PageChromeBarPeekTests`). Not reproduced: the green button not responding in fullscreen.
  > **Fixed 2026-09-25: the top bar's back button and pinned tabs under the traffic lights.** Switching to the top bar from a hidden sidebar left them 16 pt from the edge, under the lights. The bar worked out the right gap (90 pt) but only after placing its buttons, so the new gap waited for a layout pass nobody asked for. It now works the gap out first, measured against the window rather than the bar's own frame (`TopBarLightsTests`).
  > **Fixed 2026-09-25: the traffic lights came out ahead of the peek.** They were shown and hidden at once while the sidebar slid over 0.20 s, so they popped in before it arrived and vanished before it had gone. They now move inside the sidebar's own animation: they slide and fade in and out with it, and are hidden only once the fade has run (`TrafficLightPeekTests`). Same in fullscreen and windowed.
- [ ] **7.3 Row design**: favicon, title (single line, truncating), close/archive affordance on hover, audio-playing indicator + click-to-mute, loading shimmer, unread/updated dot.
  ...
  > **Fixed 2026-09-25: renaming a tab left its buttons on the name.** The close and site settings buttons stayed on top of the text being typed, in the top bar and the sidebar. They now hide while you type a name and come back after.
...
- [ ] **7.5 Folders** inside a Space (drag one tab onto another to create).
  ...
  > **Redesigned 2026-09-25: the Space pop-out.** A press on the Space's name — the sidebar's pill or the top bar's name, where a right-click does the same — opens a Luna pop-out in the site settings style instead of a native menu: the Spaces to switch to (the current one ticked), the colours as swatches, then Edit and New Space (UI-SPEC §5a, `SpacePanelTests`).
  > **Built 2026-09-25: a folded folder keeps the tab you are on.** Folding a folder while on one of its tabs leaves that tab showing under it, and going to a tab inside a folded folder (from the search bar or anywhere) shows it there without opening the folder. It stays after you click away; `⌘W` takes it away, and so does opening and folding the folder again. Both layouts. `FolderPeekTests`. Not yet checked side by side with Dia: Dia can only be looked at, not clicked, from here.
  > **Fixed 2026-09-25: top-bar folders.** A folder's tab is a normal tab's width (172 pt), so a folded folder is the same shape as a tab. An open folder's plate keeps 8 pt (the bar's gap) past its first and last tab and its tabs stand 1.5 pt in from its top and bottom — the sidebar's own two gaps — so a lit tab no longer fills the plate.
  > **Fixed 2026-09-25: the folder plate.** Its sides now line up with the other tabs' (it stood 4 pt wider), the tabs inside it end 8 pt sooner — the room a loose tab keeps from the sidebar's edge — so they sit inside it the same way, with the same 8 pt below the last one, and a folder at the top of the list no longer has its top edge cut off. The plate no longer stands 4 pt out above and below the tabs either: a folded folder's plate is exactly a tab's hover pill, with the same corners. On the top bar, a folder's plate now lights with the same wash and hairline under the pointer. UI-SPEC §3.4b, §4.
...
- [ ] **7.7 Traffic-light handling**: custom titlebar, `titlebarAppearsTransparent`, `NSWindow.toolbar` removed; traffic lights must be inset into the sidebar and must re-position correctly when the sidebar collapses, on fullscreen enter/exit, and in the Mini Window.
  ...
  > **Fixed 2026-09-26 (a pass over the lights in every flow):** `⌘S` now slides the lights with the sidebar both ways, and the first peek after hiding no longer fades them in at home (`TrafficLightPeekTests`). In fullscreen: a theme change no longer makes them vanish (`TrafficLightReapplyTests`); the titlebar band that slides down with the menu bar is hidden rather than transparent, so it no longer blocks hovering and pressing them; the hover symbols (× and the arrows) come up even when the pointer is already there after a rebuild; and the yellow light stays yellow and does nothing, instead of turning grey. Windowed, theme changes were already caught. While they fade out, a shield over them takes the press, so an invisible close button can't close the window; in fullscreen they stay in front of pop-outs and the command bar opened after them (`WindowRootView.frontmost`); and the yellow light's press comes to nothing (`LunaWindow`).
  ...
  > **Fixed 2026-09-25: no ×, − or + on the lights under the pointer in fullscreen.** A window button draws its symbol only when its parent view says the pointer is over the group (`_mouseInGroup:`). AppKit's titlebar answers that; in fullscreen the lights live in Luna's own strip, which did not. The strip now answers it. This is Luna implementing a method AppKit calls, not calling a private one — if a future macOS stops asking, the lights just go back to plain circles.
...
- [x] **9.1 Input surface** — floating rounded panel, blurred backdrop scrim, opens over the current tab; `⌘T` = new-tab mode (empty), `⌘L` = edit-current-URL mode (prefilled+selected).
  > **Fixed 2026-09-25: clicking the compact search bar on the page jumped.** The bar snapped open with no animation and the page jumped down under it. The bar now opens on its normal animation, and the Command Bar grows from the thin bar that was clicked to its full shape in the same movement (`CommandBarAnchor.startFrame`). The buttons inside the search bar now have a fully rounded hover and press shape, a little wider than tall, whose ends follow the bar's own round ends.
...
- [x] **16.4 Toolbar/action surface** — extension action buttons need a home; put them in a compact row at the sidebar bottom or in the site settings pop-out, not a fake Chrome toolbar.
  > **Built 2026-09-25 (UI-SPEC §5b).** An extensions button on every layout, and the extensions you pin beside it: at the right end of the sidebar's search bar (site settings moves to the left end), in a glass pill in the page bar's right corner, and in the top bar's capsule (the button after Downloads, pins before New Tab). Each surface shows as many pins as fit — the address keeps half the search bar, the page bar's search bar keeps half its width, the top bar keeps room for two tabs — and every extension is in the pop-out the button opens. Badges are drawn on the icons. The pop-out lists what runs in this Space with a pin on each row. Settings ▸ Extensions has a field to paste a Chrome Web Store link into (errors shown under it) and a Choose… button for a folder, `.zip` or `.crx`, then small cards two to a row: icon, name, description, what it can access in plain words, and Details, a pin and a ⋯ menu. **2026-09-26:** Details opens a popover with the whole description, everything it may do, a switch for each Space, and Chrome Web Store / Reload / Remove; the ⋯ menu also has Turn On/Off in Every Space. The "Off everywhere" Spaces button is gone. On the page bar the extensions pill is built like the back/forward pill, so its hover and press match the sidebar button beside it. New installs are not pinned. The pop-out lists every extension with a switch to turn it on or off in this Space (the pin sits before the switch, shown on hover or while pinned), and each Settings card has the same switch. Apple's iCloud Passwords cannot work in Luna — it needs a helper that macOS kills unless Safari, Chrome, Edge or Firefox starts it (checked by starting it from a shell) — and its card says so. The install and runtime permission prompts are the system alert (§16.3's prompt half). Not built: an options-page button (the manager has no API for it yet), context-menu items from extensions, and the "reading this page" indicator. Tests: `ExtensionsUITests`.
...
- [x] **17.2 Default lists**: ads + trackers + annoyances (cookie banners), each toggleable; per-site "disable blocking here" that persists in `siteSettings`.
  ...
  > **Checked 2026-09-24: built.** Toggles in `Privacy.swift`, per-site exemption in the site settings pop-out
...
- [ ] **17.5 ITP is already on** via WebKit — surface it, don't rebuild it. Add a "Clear all site data for this site" one-click action.
  > **Checked 2026-09-24: partly built.** Per-site Clear Cache / Clear Cookies in the site settings pop-out, global "Clear all site data" in Settings. Missing: ITP status and one per-site "clear all site data".
...
- [x] **23.1 Settings window — BUILT 2026-09-18.** AppKit, **not** SwiftUI and **not** tabbed: a separate `NSWindow` with a glass section list and an opaque detail pane, because only a native window can host `NSGlassEffectView`. All ten sections ship (General, Appearance, Privacy, Passwords, Search, Downloads, Shortcuts, Spaces, Extensions, Advanced); the unbuilt ones are dimmed with a one-line reason rather than hidden. Full contract in `docs/SETTINGS-SPEC.md`.
  ...
  > **Redesigned 2026-09-25: Luna Control.** The pane opens on a night sky with the switch in it: the moon is dark while apps cannot reach Luna and fills (1.1 s) when you turn it on, each connected app is a light circling it in its own colour, the one in use sends a beam to the moon each time it acts, and Connect flies a light from the app's row into orbit. The app rows have coloured planets and status dots, the permission choice is three moon-phase cards (crescent Ask, half Per Site, full Allow All), and Recent activity is a coloured timeline. Its sidebar icon is three rings joined by dotted lines, drawn to the size and weight of the other tiles' symbols, in a violet-to-cyan gradient. Claude Code's Connect runs its own `claude mcp add` (`ControlCLI` finds the tool) and only falls back to Copy Command when `claude` is not installed where its installers put it; the apps' icons ship inside Luna. `docs/LUNA-CONTROL.md` "The Settings pane"; `LunaControlSkyTests`. Not done: the sidebar moon does not fill when Luna Control is on, and the permission card is not yet in `ButtonFeedbackTests`' list (its press is tested in `LunaControlSkyTests`).
  > **Rebuilt 2026-09-25: eight pages.** Search, Downloads and Advanced are groups on General; Passwords is a group on Privacy & Passwords; Spaces, Extensions, Shortcuts, Luna Control and About stay their own pages. Rows that did nothing yet are removed ("When Luna opens", "Ask where to save each file", "Clear download list", "Show Develop menu", "Confirm before closing a window"), and so are the explanation paragraphs under groups. Every section has a black tile with its symbol in a colour (Luna Control's is three rings joined by dotted lines in a violet-to-cyan gradient, About's an "i") and every page opens on its tile, name and one line. Appearance's Tabs row is now "Sidebar side" (Left / Right, sidebar only): the top bar's Left / Centre / Right choice was removed everywhere, and its tabs always start on the left. SETTINGS-SPEC §2.2.
- [ ] **23.2 Import**:
...
- [ ] **24.1 Testing**: unit tests on frecency, hibernation policy, URL parsing/canonicalisation, blocklist conversion, traffic-light layout. UI tests for launch → command bar → navigate → split → quit → restore. A manual **Top-100-sites compat matrix** re-run each milestone (this is how we catch WebKit-vs-Chrome breakage).
  > **Checked 2026-09-24: partly built.** Unit tests for frecency, hibernation, URLs, blocking, traffic lights, tab switcher, history ranking, links from other apps, page bar, sidebar peek, window corners, the sidebar's Space pill, the Command Bar morph, the top bar's rounded page, the tab double-click and Space swipe, HTML files, one tab width, the top bar's Space-switch morph, pop-out gaps, site settings pop-out, tab rename, layout picker, updater (1005 tests). Missing: UI test target, top-100 sites check.
...
- [x] **30.9 Bottom utility bar** — profile avatar (circular, bottom-left), workspace/Space **page dots** in a small pill (centre), and a circular archive/trash button (right). The dots double as the Space switcher — adopt this instead of, or alongside, the §5.3 strip.
  ...
  > **Changed 2026-09-24: the Space pill replaced the avatar** (UI-SPEC §3.5). `SidebarSpacePill.swift`: the Space's name in a glass pill, bottom left, with its picture when it has one; a click opens the menu the avatar opened. The dots stay in the middle, on the buttons' bottom edge, and no longer have a glass pill of their own (centring them on the buttons' line was tried and reverted). The name caption over the dots is gone. The pill shows the whole name the way the top bar's does; the dots move over only for a name too long to leave them in the middle (first version cut the name to keep the dots centred, and looked squashed at 220 pt).
...
- [x] **30.12 Top-bar mode is a real second layout, not just a collapsed sidebar — CONFIRMED FOR v1 (D13)** — with the sidebar off, a single translucent bar spans the window: traffic lights → sidebar toggle → back → pinned Essentials as small tiles → **centred** URL pill → a right-hand action cluster. Tabs are not visible at all in this mode; switching happens via the Command Bar. Build it as its own layout controller, with a shared model, and animate the transition between modes.
  ...
  > **Checked 2026-09-24.** Built as §4 of `docs/UI-SPEC.md`. Since 2026-09-23: the Space name and pinned tabs on one glass plate, Space dots on hover and a two-finger swipe on the name, the page bar under the top bar (removed again 2026-09-24, see below), open folders on plates of their own with full tabs inside, pinned folders beside the Space's plate, a divider before the open tabs, new tabs at the right-hand end, the reading band on the selected tab, only empty bar space moves the window, right-click there for a new folder, and the lights held steady in full screen (d68e152 … 81dbb3d).
...
- [x] **30.14 Right action cluster** — extension action icons, `+` (new tab), downloads, and profile/account sit in their own translucent capsule, divided from the pinned tiles by a hairline. This is the natural home for the §16.4 extension buttons — build the capsule once and let both layouts host it.
  ...
  > **2026-09-25:** extension buttons are in it (§16.4).
...
- [x] **30.24 Quit sheet** — `⌘Q` asks first. `UI/Quit/QuitSheetView.swift`, `App/AppDelegate+Quit.swift` (UI-SPEC §5.2).
  > **Fixed 2026-09-25:** with Settings in front, `⌘Q` puts the sheet over Settings instead of on the browser window behind it (`QuitConfirmationTests`).
  > **Fixed 2026-09-25:** the sidebar's resize cursor no longer shows over a pop-out that covers the divider (`SpacePanelTests`). Extension switches in Settings and in the pop-out no longer flash when turned off: the card or row is updated instead of rebuilt mid-slide (`ExtensionsUITests`). Settings rows whose title already says what they do lost their subtitles (SETTINGS-SPEC §2).
  > **Fixed 2026-09-26 (CI):** Luna Control's typing tests waited a fixed 0.15–0.3 s for the page and failed on the slower CI runner; they now wait until the text has arrived. No app test reads the checkout any more — the Command Bar offline check is `Tools/check-command-bar-offline.sh` (CI and `make check`) and the extension fixture is written to a temp folder — so a test host without Documents access no longer hangs.
  > **Fixed 2026-09-26:** a control on the edge of a Settings page (the Allow All card, New Space) was cut off while it swelled on a click; the page's margins are now inside the scroll area, on every side (`SettingsPaneSwellRoomTests`). Luna Control: the paragraph under the three mode cards is gone, and Allowed sites is shown only in Per Site mode (docs/LUNA-CONTROL.md).
...
- [x] **30.27 Page bar** — the address, back and reload over the page, in the sidebar layout. `UI/Browser/PageChromeBar.swift` (UI-SPEC §3.2b).
  > **Changed 2026-09-24: gone from the top bar layout.** A click on the tab on screen opens its address in the Command Bar, as `⌘L` does, and a double-click renames the tab in place (UI-SPEC §4). No back or forward buttons in that layout; the sidebar layout comes back from Settings ▸ Appearance ▸ Layout.
...
- [x] **30.29 Chrome density** — Clear / Opaque glass. `Design/GlassDensity.swift` (UI-SPEC §2a).
  > **Removed 2026-09-24.** Clear is the only material; the Material row, `Glass.density`, `Surface.frostOpaque` and `Surface.popoverFrostOpaque` are gone. A stored `appearance.glassDensity` is ignored. UI-SPEC §2a keeps the measurement.
- [x] **30.30 Window corners** — the reference's 25 pt by default, macOS's own 16 pt under Settings ▸ Appearance ▸ Match macOS corners, with the page's corners following (UI-SPEC §3.6). `WindowCorner` in `UI/Window/WindowRootView.swift`.
  > **Built 2026-09-24.** The Command Bar, pop-outs, tab switcher and quit sheet keep 25 pt (`panelCornerRadius`).
- [ ] **30.31 Sidebar flicker in Stage Manager** — the sidebar flickers as the window leaves and enters the stage (UI-SPEC §2).
  > **Investigated 2026-09-24, not fixed.** Not the corners. Measured at 120 fps: the sidebar comes up lighter when Luna loses focus (29.4 → 42.2 dark), and one flat frame about 0.4 s after the window returns is macOS's own — a bare glass test window shows it too, and an opaque window does not. Painting the tint on Luna's plate in every state removed the lightening, but the flicker looked the same, so it was reverted. Worth a Feedback Assistant report with the test window.
...
| **A peeked sidebar stays out while its Downloads or History list is open** (§7.2) | Until the list is closed, not until the pointer leaves the sidebar. The list never brings a hidden sidebar out by itself. |
| **No page bar under the top bar** (§30.27, UI-SPEC §4) | Clicking the tab on screen edits its address in the Command Bar, which grows out of the tab the way it grows out of the sidebar's address bar; double-clicking a tab renames it. |
| **Top bar: back and forward before the tabs, the Space on the right** (UI-SPEC §4) | The Space's name moves from the head of the pinned plate to its own capsule after History and Downloads; the profile button is gone. |
| **Top bar: the page's top corners are rounded, and the bar has no glass of its own** (UI-SPEC §3.6, §4) | Reverses §4's "flush full-bleed, no inset card": the page rounds its two top corners against the bar, as it does against the sidebar. The bar's own glass sheet (and the line along its bottom) is gone; it sits on the window's glass. |
| **The Command Bar opens from a tab at once, and morphs** (UI-SPEC §4, §9.1) | A click on the tab on screen opens it immediately instead of after the double-click wait; a second click folds it back and renames. The bar grows out of the tab's own frame and corner (width, place, height together, 0.26 s) and folds back into it; the sidebar and page-bar pills morph the same way. |
| **Top bar: Space swipes move the tabs; new tabs stay in view; the Command Bar stays inside the window** (UI-SPEC §4) | The run of tabs slides with a Space swipe and the next Space's slides in; a new tab is followed as its title widens it; an opened Command Bar never rises past the window's top edge, from the tab or from either pill. |
| **Every top bar tab is one width** (UI-SPEC §4) | 172 pt, Dia's, measured; a longer title fades. Tabs were as wide as their titles, 120–180 pt. Folder headers still fit their names. |
| **Luna opens HTML files, in the Luna that is running** | `Info.plist` claims HTML documents (as an alternate handler, not the default) and a file opened with Luna opens as a tab; before, it was dropped. A second copy of Luna started by macOS to open a file hands it to the running one and quits instead of opening a second window on the same database. |
| **The top bar's Space name slides with a swipe** (UI-SPEC §4, §30.9) | The name slides out with the tabs and in from the other side. No haptic: one was added and taken out the same day. |
| **Top bar: a Space switch morphs instead of jumping** (UI-SPEC §4) | The Space capsule changes width to the new name, and the buttons beside it and the tabs' edge move with it; the pinned-tabs plate changes width too. A click switch fades the old tabs out and the new ones in. |
| **Top bar: 50 pt tall, tabs 4 pt apart** (UI-SPEC §4) | The same 7 pt above and below a tab (it was 7 and 9). Open tabs stand Dia's 4 pt apart instead of 8. |
| **Top bar: History and Downloads open 5 pt off the capsule, as in the sidebar** (UI-SPEC §4) | The gap was measured from the button, which sits 4 pt inside its capsule, so the list stood 9 pt off the glass. |
| **Site settings are a pop-out, and the top bar has them on the selected tab** (UI-SPEC §3.2a, §4) | The site menu (`NSMenu`) is now a glass pop-out like History and Downloads, from the sidebar's pill, the page bar's and the top bar. Header says whether the connection is secure (green); Block Ads & Trackers, Automatic Picture-in-Picture and Local Network are switches; Share, Copy Link, Clear Cache, Clear Cookies and More Settings are rows. The selected top-bar tab shows the sliders glyph next to its close ×. |
| **Every switch is the macOS Liquid Glass switch** (SETTINGS-SPEC §1.2) | Settings and the site settings pop-out use `NSSwitch` (`SystemSwitch`) instead of Luna's hand-drawn 36 × 20 copy. |
| **Site settings stand centred on their button; icon tabs get them on right-click** (UI-SPEC §3.2a, §4) | The pop-out is centred across the glyph instead of hanging from its left edge. A kept tile's right-click menu has *Site Settings…*, which selects the tab first if needed. The sliders and × on a tab now line up by their ink (they were 0.75 pt apart). |
...
| **Windows can wear macOS's corner** (§30.30) | Settings ▸ Appearance ▸ Match macOS corners: 16 pt instead of the default 25. The page's corners follow. |
