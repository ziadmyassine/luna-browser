...
- [ ] **23.2 Import**:
  ...
  - **Arc's Spaces are not recreated.** §3.4b puts one import in one folder named after the browser, so the four of them flatten into `Arc`. Recreating them means a second answer to "where does an import land", and that is a decision, not a reader.
