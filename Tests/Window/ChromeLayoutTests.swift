...
final class TrafficLightLayoutTests: XCTestCase {
    ...
    /// Fullscreen has to land the lights where the titlebar lands them
    /// windowed. There the lights stand at AppKit's own origins inside a strip
    /// of Luna's, and the strip is placed so those origins are the windowed
    /// placement — so AppKit resetting one of them, which it does on every
    /// reveal of the menu bar, moves it nowhere rather than into a staircase.
    func testTheFullscreenStripPutsAppKitsOriginsWhereTheWindowedLightsAre() throws {
        let inset = Tokens.Metric.trafficLightInset
        let windowed = try XCTUnwrap(origins(.sidebar(width: 280, edge: .leading), inset: inset))
        let root = NSRect(x: 0, y: 0, width: 1512, height: 949)
        let strip = TrafficLightLayoutManager.stripFrame(
            in: root,
            natural: system.natural[0],
            buttonHeight: system.buttonHeight,
            inset: inset
        )
        for (natural, placed) in zip(system.natural, windowed) {
            // The windowed origin is in the titlebar, which hangs from the
            // window's top; the fullscreen one is natural, inside the strip.
            XCTAssertEqual(strip.minX + natural.x, root.minX + placed.x)
            let fromTop = root.maxY - (strip.minY + natural.y + system.buttonHeight)
            XCTAssertEqual(fromTop, inset)
        }
    }
