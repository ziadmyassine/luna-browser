...
enum ShortcutResults {
    ...
    /// Nil when the command does not answer, and otherwise which tier it
    /// answers in and how squarely.
    ///
    /// Token-AND over the title and the keywords, the same rule everything
    /// else in the list is matched by. The tier is the first question: a
    /// command whose own name said nothing sits below the search row
    /// (`.keywordShortcut`), because the word that found it is one the row
    /// does not show.
    ///
    /// What the number says, in order: a title the query starts beats one it
    /// does not; between two of those, the one the query covers more of; below
    /// them, the more of the query the title itself answered rather than the
    /// keywords; and last, the shorter title.
    private static func match(_ tokens: [String], for entry: ShortcutEntry) -> (source: CommandBarSource, score: Double)? {
        let title = entry.title.lowercased()
        let found = tokens.allSatisfy { token in
            title.contains(token) || entry.keywords.contains { $0.contains(token) }
        }
        guard found else { return nil }
        // The shorter title, between two that answered as much. "copy link" is
        // half a title match for both Copy URL and Copy URL as Markdown, and
        // Copy URL is the one being asked for.
        let tieBreak = 1 / Double(title.count + 1)
        let inTitle = Double(tokens.count { title.contains($0) }) / Double(tokens.count)
        guard inTitle > 0 else { return (.keywordShortcut, tieBreak) }
        ...
        if title.hasPrefix(query) { return (.shortcut, 2 + Double(query.count) / Double(title.count)) }
        // Otherwise, how much of the query the title itself answered — the rest
        // came from the keywords.
        return (.shortcut, inTitle + tieBreak)
    }
}
