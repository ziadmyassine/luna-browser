...
final class DownloadManager {
    ...
    /// Newest first — the order both the popover and the list panel read.
    ...
    /// Where §5's popover points: the downloads button in the top bar's action
    /// capsule. The host (agent J) supplies it; downloads work without one, the
    /// popover just does not appear.
    var anchorProvider: (() -> NSView?)?
    ...
    private let popover = DownloadsPopover()
    ...
    fileprivate func finished(_ item: DownloadItem, download: WKDownload) {
        ...
        onChange?()
        // §5: the popover is the primary surface, and it only ever appears on
        // completion. A failure belongs in the list panel, not over the window.
        if let anchor = anchorProvider?() {
            popover.present(item, anchoredTo: anchor)
        }
    }
