...
final class DownloadsPanelController: PopoutController {
    ...
    /// The Space the given window is showing, asked fresh every time the list
    /// is built or reloaded — §15.3's list is one Space's (§9.2), and the Space
    /// can change while the panel is shut. Asked of the window the list stands
    /// in because one controller serves every window, a §5.6 one included.
    /// Nil until the host wires it, which shows the downloads that have no
    /// Space either.
    var activeSpace: ((NSWindow) -> UUID?)?

    /// The rows for the window the list is standing in.
    var items: [DownloadItem] { manager.items(inSpace: spaceID) }

    private var spaceID: UUID? { host.flatMap { activeSpace?($0) } }
    ...
    override func makePanel(in root: NSView) -> PopoutPanelView {
        ...
        panel.onClear = { [weak self] in self?.manager.clearCompleted(inSpace: self?.spaceID) }
        return panel
    }
    ...
    private func reload() {
        (presented as? DownloadsPanel)?.setItems(items)
    }
}
