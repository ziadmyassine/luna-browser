...
- **The selected row shows how far its page has been read.** A band of `Surface.readBand` (6 %) lies
  ...
  > The page reports the fraction through the same script as §3.2b's offset. When the document itself
  > does not scroll, it uses the last element that scrolled, if that element is at least half the viewport
  > tall. That covers app-shell sites without a dropdown list driving the band.
