...
//  Colours cross that boundary as `RGBA` because `BrowserKit` must stay
//  portable (contract rule 2, enforced by `Tools/check-no-appkit.sh`); without
//  this file every view would re-do the conversion by hand.
//
//  §8.2's artwork — the 12 curated pairs, the Space badge, the sidebar wash,
//  the content-edge glow — is deliberately **not** here. This is the bridge and
//  nothing else.
//
...
extension NSGradient {
    ...
    /// never do — so a `!` at the call site is safe if you would rather not
    /// unwrap. The initialiser stays failable because `NSGradient`'s is.
