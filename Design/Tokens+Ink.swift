...
//  when it crossed the 400-line file limit.
//
...
//  stay in that file. Alphas are numbers; they travel.
//
...
extension Tokens {
    ...
    /// The alpha table behind the translucent tokens, in one place: the tokens
    /// build their colours from it and `TokenCheck` re-measures the Increase
    /// Contrast variants from it. That second reader is why this is not
    /// private — the contrast branch is unreachable through an `NSAppearance`
    /// on macOS 26 (see `Accessibility.swift`), so a check that could not read
    /// these alphas could not verify §2 at all.
    enum Ink {
        ...
        /// §3.4's 6 %. Doubled under Increase Contrast, where a 6 % wash is the
        /// first thing to vanish.
        static let hover = InkAlphas(light: 0.06, dark: 0.06, contrastLight: 0.12, contrastDark: 0.12)
        /// §3.4's selected pill, twice `hover` so the two stay separable. Capped
        ...
        /// §2's chrome tint — see `Surface.glassTint`. Well short of opaque: the
        /// chrome samples the desktop, and a tint heavy enough to hide that
        /// would be a coloured rectangle. Increase Contrast thickens it.
        ...
        /// still legibly there behind the chrome. Thickens toward opaque under
        /// Increase Contrast, for the same reason the tint does.
        static let frost = InkAlphas(light: 0.46, dark: 0.50, contrastLight: 0.68, contrastDark: 0.72)
        /// `Surface.well` — a dormant control's recess. Deep in dark mode so it
        /// reads as cut into the plane rather than drawn on it; light mode needs
        /// far less, because a light surface shows a darkening at a much lower
        /// alpha.
        ...
        /// gains under Increase Contrast, because "disabled" has to stay legible
        /// as a control even when it is not readable as text.
