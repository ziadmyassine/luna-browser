...
//  Luna
//
//  The three (four, counting colour) display settings the chrome has to obey,
//  read live. Split out of `Tokens.swift` for its length limit, and it earns
//  the separation: nothing here is a colour, and half the app asks these
//  questions without ever asking for a token.
//
//  The one finding that shapes every caller, repeated here because this is
//  where people will look for it: **on macOS 26.5 Increase Contrast is not an
//  `NSAppearance`.** `NSAppearance(named: .accessibilityHighContrastAqua)`
//  hands back the *identical object* as `.aqua` (verified: `===`), so no
...
//  is why the tokens branch on it at resolve time — and why **a view that
//  draws text or hairlines must redraw on
//  `NSWorkspace.accessibilityDisplayOptionsDidChangeNotification`**, not merely
